...
uint8_t icm_20948_read_data(icm_20948_data* data)
{
	...
	data ->x_gyro = (((int16_t)data_rx[6]<<8) | (int16_t) data_rx[7]) - GYRO_X_BIAS;
	data ->y_gyro = -(((int16_t)data_rx[8]<<8) | (int16_t)data_rx[9]) - GYRO_Y_BIAS;
	data ->z_gyro = -(((int16_t)data_rx[10]<<8)| (int16_t)data_rx[11]) - GYRO_Z_BIAS;
