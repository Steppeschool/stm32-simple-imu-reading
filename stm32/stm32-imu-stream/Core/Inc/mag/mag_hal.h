...
/*
 * Base handle — must be the first member of every driver-specific mag handle.
 *
...
typedef struct {
    ...
    int8_t (*data_ready)(mag_handle_t *h, uint8_t *ready);
...
static inline int8_t MAG_DataReady(mag_handle_t *h, uint8_t *ready)
    { return h->driver->data_ready(h, ready); }
