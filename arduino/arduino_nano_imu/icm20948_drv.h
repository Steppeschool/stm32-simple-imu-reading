 ...
 *
 *   imu_handle_t * â†’ (icm20948_handle_t *)  for IMU_ReadRaw() etc.
 *   mag_handle_t * â†’ &handle->mag           for MAG_ReadRaw() etc.
 *
...
typedef struct {
    imu_handle_t      base;          /* MUST be first â€” cast target for IMU API  */
    ...
    uint8_t           spi_mode;      /* 1 = SPI bus, 0 = I2C bus                 */
} icm20948_handle_t;
